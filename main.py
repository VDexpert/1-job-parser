...
def starter():
    ...
    hh_vac = StackVacancies()
    sj_vac = StackVacancies

    print("\n", "*"*10, "Парсинг данных с сайта hh.ru", "*"*10, "\n")
    for page in range(1, range_hh):
        hh_engine.get_request(page)

    data_hh = Connector.all_connectors[hh_engine.filename].connect()
    count_hh = 0
    print("\n", "*" * 10, "Форматирование данных с сайта hh.ru", "*" * 10, "\n")
    for vac in data_hh:
        count_hh += 1
        hh_vac.push(HHVacancy(name=vac["name"], href=vac["href"], company=vac["company"], experience=vac["experience"],
                  salary=vac["salary"], description=vac["description"], filename=hh_engine.filename))
        if count_hh == quantity_vac:
            break

    outfile_hh = f"HH-vacancies-{hh_engine.key_word}.csv"
    print("\n", "*" * 10, f"Загрузка данных в файл {outfile_hh}", "*" * 10, "\n")
    write_vac_to_outfile(hh_vac.to_list(), outfile_hh, HHVacancy.get_count_of_vacancy(hh_engine.filename))
    ...
    data_sj = Connector.all_connectors[sj_engine.filename].connect()
    count_sj = 0
    print("\n", "*" * 10, "Форматирование данных с сайта superjob.ru", "*" * 10, "\n")
    for vac in data_sj:
        count_sj += 1
        sj_vac.push(SJVacancy(name=vac["name"], href=vac["href"], company=vac["company"], experience=vac["experience"],
                  salary=vac["salary"], description=vac["description"], filename=sj_engine.filename))
        if count_sj == quantity_vac:
            break

    outfile_sj = f"SJ-vacancies-{sj_engine.key_word}.csv"
    print("\n", "*" * 10, f"Загрузка данных в файл {outfile_sj}", "*" * 10, "\n")
    write_vac_to_outfile(sj_vac.to_list(), outfile_sj, SJVacancy.get_count_of_vacancy(sj_engine.filename))
    ...
    if additionals == "1":
        ...
        write_vac_to_outfile(sorting(hh_vac.to_list(), order), outfile_hh_sort, quantity_vac)
        print("\n", "*" * 10, f"Отсортированные вакансии в файле HH-sort-in-{order}-vacancies.csv", "*" * 10, "\n")

        outfile_sj_sort = f"SJ-sort-in-{order}-vacancies.csv"
        write_vac_to_outfile(sorting(sj_vac.to_list(), order), outfile_sj_sort, quantity_vac)
        ...
        top_vacancies_hh = get_top(hh_vac.to_list(), top_count, orders["1"])
        ...
        top_vacancies_sj = get_top(sj_vac.to_list(), top_count, orders["1"])
        ...
        selecting_hh = Connector.all_connectors[hh_engine.filename].select(query_sel)
        write_vac_to_outfile(selecting_hh, outfile_hh_select, HHVacancy.get_count_of_vacancy(hh_engine.filename))
        ...
        selecting_sj = Connector.all_connectors[sj_engine.filename].select(query_sel)
        write_vac_to_outfile(selecting_sj, outfile_sj_select, SJVacancy.get_count_of_vacancy(sj_engine.filename))
        ...
        contain_id_exper = {"1": "Нет опыта", "2": "от 1 года до 3 лет", "3": "от 3 до 6 лет", "4": "более 6 лет"}
        if del_exper.replace(" ", "") == "":
            del_exper = "Нет опыта"
        else:
            del_exper = contain_id_exper[del_exper.replace(" ", "")]
        ...
        without_deleting_hh = Connector.all_connectors[hh_engine.filename].delete(query_del)
        write_vac_to_outfile(without_deleting_hh, outfile_hh_del, HHVacancy.get_count_of_vacancy(hh_engine.filename))
        ...
        without_deleting_sj = Connector.all_connectors[sj_engine.filename].delete(query_del)
        write_vac_to_outfile(without_deleting_sj, outfile_sj_del, SJVacancy.get_count_of_vacancy(sj_engine.filename))
        print("\n", "*" * 10, f"Данные удалены. Нужные данные в файле {outfile_sj_del}", "*" * 10, "\n")

    del_temp_json(hh_engine.filename)
    del_temp_json(sj_engine.filename)
